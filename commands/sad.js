...
module.exports = {
    ...
    run: async(client, message, args) => {
        ...
        if(user.id === message.author.id) return sendError(`**Cheer up! Don't be sad at yourself.**` , message.channel);
        const jsonsad = fs.readFileSync(
          './roleplay.json'
          );
         const sadArray = JSON.parse(jsonsad).sad;
